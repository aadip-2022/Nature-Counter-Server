...
module.exports = {
    getAllEntriesByUserId: async (req, res, next) => {
        try {
            ...
            if (id){
                const entries = await Journal.find({ uid: id });
                res.json(entries);
            }
        } catch (err) {
        ...
        try {
            ...
            if (id){
                ...
                const entries = await Journal.aggregate([
                    ...
                    {$project:{uid: "$uid", duration: {$divide: [{$subtract: ["$end_time", "$start_time"]}, 1000]}}},
                    {$group: {_id: "$uid", totalMin: {$sum : "$duration"}}}
        ...
        try {
            ...
            if (id){
                ...
                const entries = await Journal.aggregate([
                    {$match:{uid: id}},
                    {$match:{start_time: {$gte: new Date(startDate), $lte: new Date(endDate)}}},
                    {$project:{uid: "$uid", start_time:"$start_time",duration: {$divide: [{$subtract: ["$end_time", "$start_time"]}, 1000]}}},
